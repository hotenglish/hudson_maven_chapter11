...
public class AccountPersistServiceImpl implements AccountPersistService {
    ...
    public Account createAccount(Account account) throws AccountPersistException {
        ...
        for (Element item : (List<Element>) accountsEle.elements()) {
            id = Integer.parseInt(item.elementText(ELEMENT_ACCOUNT_ID));
        }
        id++;
        Element element = new UserDataElement(ELEMENT_ACCOUNT);
        element.addElement(ELEMENT_ACCOUNT_ID).setText(String.valueOf(id));
        ...
        element.addElement(ELEMENT_ACCOUNT_ACTIVATED).setText(Boolean.toString(account.isActivated()));
    ...
    public Account updateAccount(Account account) throws AccountPersistException {
        ...
        for (Element accountEle : (List<Element>) accountsEle.elements()) {
            if (accountEle.elementText(ELEMENT_ACCOUNT_ID).equals(account.getId())) {
                ...
                accountEle.element(ELEMENT_ACCOUNT_ACTIVATED).setText(Boolean.toString(account.isActivated()));
            }
        }
        writeDocument(doc);
        return account;
    ...
    private void writeDocument(Document doc) throws AccountPersistException {
        ...
        try {
            ...
            try {
                ...
                throw new AccountPersistException("Unable to close persist data writer", e);
            }
        }
    }
